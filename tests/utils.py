...
def generate_training_corpus(
    ...
    for _ in range(num_sentences):
        sentence_size = rs.randint(sentence_min_size, sentence_max_size)
        yield [str(x) for x in rs.randint(0, vocabulary_size, sentence_size)]
...
def build_coocurrence_matrix(sentences):
    ...
    for sentence in sentences:
        for i, first_word in enumerate(sentence):
            first_word_idx = dictionary.setdefault(first_word, len(dictionary))
            for j, second_word in enumerate(sentence[i : i + window + 1]):
                second_word_idx = dictionary.setdefault(second_word, len(dictionary))
